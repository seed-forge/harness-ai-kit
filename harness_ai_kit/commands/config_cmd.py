"""``harness-ai-kit config`` command: show effective config for an asset."""
...
from harness_ai_kit.infrastructure.config_io import resolve_repo_root
from harness_ai_kit.infrastructure.config_io import load_config as load_cli_config
...
def _default_env_tak_path() -> Path:
    return Path.home() / ".harness-ai-kit" / ".env.tak"
...
def command_config(args: argparse.Namespace, config_path: Path) -> int:
    ...
    if missing:
        print(f"\nUnconfigured required fields (set in ~/.harness-ai-kit/config.yaml or .env.tak):")
        for line in missing:
            print(line)
    return 0
