...
def _get_shared_resources_path() -> Path:
    ...
    return Path.home() / ".harness-ai-kit" / "shared-resources.yml"
...
def command_shared_resources_list(args: argparse.Namespace, config_path: Path) -> int:
    ...
    if not resources:
        ...
        print(f"使用 'harness-ai-kit shared-resources add <type>.<name>' 添加资源")
        return 0
...
def command_shared_resources_use(args: argparse.Namespace, config_path: Path) -> int:
    ...
    if resource is None:
        ...
        print(f"使用 'harness-ai-kit shared-resources list' 查看可用资源")
        return 1
