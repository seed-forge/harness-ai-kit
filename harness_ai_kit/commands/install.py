...
from harness_ai_kit.domain.identity import package_key_for, split_canonical_id
from harness_ai_kit.domain.lockfile import Lockfile
...
from harness_ai_kit.domain.project_manifest_state import remove_asset_from_manifest
from harness_ai_kit.domain.models import ProjectManifest
from harness_ai_kit.domain.models.constants import PROJECT_MANIFEST_SCHEMA_VERSION
...
def command_sync(args: argparse.Namespace, config_path: Path, context: InstallCommandContext) -> int:
    ...
    if mode == "cli":
        ...
        if not args.dry_run and any(context.is_self_cli_package(record.package_name) for record in records):
            ...
            raise ValueError(
                "Refusing to install or upgrade harness-ai-kit from a running harness-ai-kit process on Windows. "
                f"Run this manually instead: {recovery}. "
                "If you also want the operator workflow skill, install it separately with "
                "`harness-ai-kit install skill harness-ai-kit-ops`."
            )
    ...
    if mode == "loop":
        ...
        if not asset_ids:
            raise ValueError("No loop ids specified. Pass loop ids, e.g. `harness-ai-kit install loop batch-migration-loop`.")
        ...
        if args.dry_run:
            ...
            for node in cli_dep_nodes:
                print(f"  - requires CLI {node.canonical_id} {node.version} (install separately: harness-ai-kit install cli {node.id})")
            if unresolved_deps:
        ...
        for node in cli_dep_nodes:
            print(f"note: requires CLI {node.canonical_id} {node.version} (install separately: harness-ai-kit install cli {node.id})")
        if unresolved_deps:
            print(f"WARNING: Skipped unresolved dependencies: {', '.join(unresolved_deps)}")
            print("Run `harness-ai-kit install skill <id>` for each once they become available.")
        return 0
    ...
    if runtime_priority is not None and runtime_id in runtime_priority:
        from harness_ai_kit.domain.runtime_install import should_skip_for_priority
