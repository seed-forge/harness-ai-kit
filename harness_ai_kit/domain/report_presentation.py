...
def validation_summary_lines(*, has_error: bool) -> list[str]:
    if has_error:
        return [
            ...
            "Next: fix the reported metadata, version drift, or entry-file issues, then rerun `harness-ai-kit validate`.",
        ]
    return [
        "Success: repository validation passed.",
        "Next: run `harness-ai-kit publish ...` when you are ready to release changes.",
    ]
...
def sync_repo_success_lines(*, message: str) -> list[str]:
    return [
        f"Success: {message}",
        "Next: run `harness-ai-kit list` or `harness-ai-kit install ...`.",
    ]
...
def list_empty_lines(*, subject: str, repo_available: bool) -> list[str]:
    if subject in {"skill", "skills"}:
        next_hint = (
            "Add a skill under `skills/` and rerun `harness-ai-kit validate`."
            if repo_available
    ...
    if subject in {"cli", "clis"}:
        next_hint = (
            "Add `cli/<name>/cli.json` and rerun `harness-ai-kit list clis`."
            if repo_available
    ...
    next_hint = (
        "Create one with `harness-ai-kit create skill <id>` or `harness-ai-kit create cli <id>`."
        if repo_available
...
def list_update_available_line() -> str:
    return "Next: run `harness-ai-kit update` to refresh outdated installed assets."
...
def doctor_runtimes_next_line() -> str:
    return "Next: use `harness-ai-kit install skill <id> --runtime codex --scope project` or add a runtime adapter."
...
def doctor_sources_success_hint_line() -> str:
    return "Hint: run `harness-ai-kit sync-repo` or `harness-ai-kit upgrade --all` if the selected source looks stale."
...
def doctor_next_line(*, repo_available: bool) -> str:
    next_hint = "harness-ai-kit list" if repo_available else "harness-ai-kit bootstrap --repo-url <git-url>"
