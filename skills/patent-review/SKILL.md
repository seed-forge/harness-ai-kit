...
| D7 | 图文一致性 | 附图中的节点文本、连线方向、步骤编号是否与正文一致（需 `.drawio` 或 `.xml` 附图文件，由 `drawioctl inspect --json` 取证） | 附图与正文描述不一致 |
...
- **图文审查为可选特性**：D7 维度需用户提供 `.drawio` 或 `.xml` 附图文件。启用时先运行 `drawioctl inspect <file> --json` 取得全页节点/连线清单，再与正文交叉校验。若 inspect 报结构错误，先报告附图结构问题，不把它误判为正文语义不一致；无附图时自动跳过该维度。
