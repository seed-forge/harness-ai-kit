...
## 0.1.0 (draft)
- 初始草案（C 类纵向）。由 base-historyminer-ops 从 ac-linkmind NL2SQL 会话(277)深挖提炼。
- 知识基座，模型参照 public-mysql-expert-base；含 architecture / understanding / correctness-eval 三份 reference。
- **命名说明**：定稿蓝图 v2 曾记为 `devlab-nl2sql-expert`；按 team-ai-kit 知识基座约定（`public-*-expert-base`）对齐为 `public-nl2sql-expert-base`，最终命名待管理员确认。
