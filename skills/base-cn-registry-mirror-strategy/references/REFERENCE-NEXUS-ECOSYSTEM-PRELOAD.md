...
组织内部集群 在 **Kaniko + Woodpecker + 跨境网络** 场景下，仅把构建指向国内公共镜像站（npmmirror、阿里云 Maven、goproxy.cn 等）往往**仍然很慢**：流量仍走公网 CDN、大制品重复下载、镜像站同步滞后、且经 Clash 代理时易 502/超时。
...
PyPI 已按此模式落地，见 `组织内部集群/ansible-control/artifacts/nexus-pypi/README.md`；其它生态应对齐同一抽象，而不是各写各的「换阿里云」。
...
4. **构建在代理环境**：Woodpecker/Kaniko 配 `HTTP_PROXY` 时，外网 registry 常比内网 Nexus **更慢或更脆**；`NO_PROXY` 须含 `<nexus-url>`（及 `.<service-url>`）。
...
| 策略 | 适用 | 动作 |
|------|------|------|
| **cache** | pinned 版本已在 group simple/index 可见 | 在 <host> 经 group 执行 `pip download` / `mvn dependency:get` 等，填满 proxy 缓存 |
...
| 生态 | 构建消费端配置 | group 入口形态（需在 Nexus 配置） | 预热手段（hosted 优先时） | 组织内部集群 现状（2026-05） |
|------|----------------|-----------------------------------|---------------------------|-------------------------|
...
**结论**：CN 镜像写进 `ARG` 默认值适合「没有 Nexus」的模板；**组织内部集群 `deploy-apps` + Woodpecker 标准路径**应把默认值改为 **Nexus group**，大依赖用 **manifest 预热** 补齐。
...
| 用途 | 路径 |
|------|------|
| PyPI 预热 playbook | `组织内部集群/ansible-control/playbooks/nexus_pypi_wheels_sync.yml` |
| PyPI manifests | `组织内部集群/ansible-control/artifacts/nexus-pypi/manifests/` |
| deploy-apps 示例 | `组织内部集群/gitea-repos/deploy-apps/*-deploy/prepare-upstream.sh`、`.woodpecker.yml` `no_proxy` |
| 公共换源片段（L2） | `base-cn-registry-mirror-strategy/references/REFERENCE-MIRROR-RECIPES.md` |
| 出站代理 | `组织内部集群-组织内部集群-ops/references/REFERENCE-组织内部集群-OUTBOUND-HTTP-PROXY.md` |
...
本文件为**策略与决策**事实源；具体 Nexus 仓库名以 `<host>` Nexus UI / 运维台账为准，配置变更时同步更新上表「group 入口」列。
