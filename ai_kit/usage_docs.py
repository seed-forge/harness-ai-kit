...
def _render_ai_kit_operator(metadata: Mapping[str, Any]) -> str:
    asset_id = _clean_text(metadata.get("id") or "ai-kit-ops")
    name = _clean_text(metadata.get("name") or metadata.get("id") or "ai-kit-ops")
    entry = _clean_text(metadata.get("entry") or "SKILL.md")
    when = [
        "- Use this skill when you need to operate `ai-kit` from the member side: install, list, show, cat, sync, doctor, create, submit, or upgrade.",
    ...
    outputs = [
        "- A ready-to-run `ai-kit` command sequence.",
    ...
    prompts = [
        _prompt_block(
            "场景 1：安装或升级资产",
            [
                f"请使用 `{asset_id}` 帮我处理 ai-kit 资产安装或升级。",
    ...
    fast_path = [
        "- Run `ai-kit doctor` first.",
        f"- Use `ai-kit show <type> <id>` or `ai-kit cat <type> <id>` to inspect, then open `{entry}` for the full command flow.",
...
def _render_ai_kit_maintainer(metadata: Mapping[str, Any]) -> str:
    asset_id = _clean_text(metadata.get("id") or "ai-kit-maintainer")
    name = _clean_text(metadata.get("name") or metadata.get("id") or "ai-kit-maintainer")
    entry = _clean_text(metadata.get("entry") or "SKILL.md")
    when = [
        "- Use this skill when you need to change `ai-kit` repository facts: metadata, catalog, templates, validation, or sync behavior.",
    ...
    prompts = [
        _prompt_block(
            "场景 1：维护 ai-kit 资产",
            [
                f"请使用 `{asset_id}` 帮我修改 ai-kit 仓库里的资产治理内容。",
...
def _render_ai_kit_cli(metadata: Mapping[str, Any]) -> str:
    name = _clean_text(metadata.get("name") or metadata.get("id") or "ai-kit")
    ...
    fast_path = [
        "- Run `ai-kit doctor` first.",
        "- Use `ai-kit validate` before submit or release, then read `README.md`, `INSTALL.md`, and `RELEASE.md` for the full flow.",
...
def render_usage_doc(metadata: Mapping[str, Any]) -> str:
    ...
    if package_type == "skill" and asset_id == "ai-kit-ops":
        return _render_ai_kit_operator(metadata)
    if package_type == "skill" and asset_id == "ai-kit-maintainer":
        return _render_ai_kit_maintainer(metadata)
    if package_type == "cli" and asset_id == "ai-kit":
    ...
    if package_type == "cli":
        ...
        inputs = [
            ...
            "- Config: read `data/config.defaults.yaml` for key declarations, then `~/.ai-kit/config.yaml` (`assets.<cli-id>` section) for user values.",
        ]
        outputs = [
        ...
        fast_path = [
            ...
            "- Config discovery: `data/config.defaults.yaml` → `~/.ai-kit/config.yaml` (assets section) → ask user for missing required keys.",
        ]
