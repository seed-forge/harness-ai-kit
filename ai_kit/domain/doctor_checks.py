...
from ai_kit.domain.models import CliAssetRecord, CliConfig, SkillRecord
...
from ai_kit.application.project_sync import resolve_skill_plan
from ai_kit.domain.validation import (
    validate_cli_companion_docs,
    validate_companion_docs,
)
from ai_kit.infrastructure.cli_installer import catalog_versions
...
from ai_kit.domain.versions import compare_versions
...
def doctor_versions_results(repo_root: Path) -> list[dict[str, str]]:
    ...
    cli_root_record = cli_inventory.get("ai-kit")
    ...
    for relative_path in ("README.md", "INSTALL.md"):
        ...
        if not document_path.exists():
            results.append(
                {
                    "subject": f"cli:ai-kit:{relative_path}",
                    "status": "error",
        ...
        expected = f"ai-kit=={project_version}"
        results.append(
            {
                "subject": f"cli:ai-kit:{relative_path}",
    ...
    for skill_id in ("ai-kit-ops", "ai-kit-maintainer"):
...
def check_bindings(cwd: Path) -> list[dict[str, str]]:
    ...
    if shared_refs:
        global_sr_path = Path.home() / ".ai-kit" / "shared-resources.yml"
        global_sr: dict = {}
...
def doctor_sources_payload(
    ...
    for root_id in root_ids:
        ...
        if repo_version and registry_version and compare_versions(repo_version, registry_version) < 0:
            warnings.append(
                f"{root_id}: local ai-kit checkout is older than registry ({repo_version} < {registry_version})"
            )
    ...
    if repo_root is None:
        warnings.append("ai-kit repository root is not available on this machine.")
