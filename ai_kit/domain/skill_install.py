...
from ai_kit.domain.lockfile_io import topological_skill_nodes_from_lock
from ai_kit.domain.policies import SOURCE_GIT_REPO, SOURCE_PUBLIC_REGISTRY, SOURCE_REGISTRY, SOURCE_REPO
from ai_kit.domain.runtime_install import runtime_install_destination, runtime_profile
...
def _resolve_installed_skill_dir(
    ...
    from ai_kit.domain.runtime_install import runtime_asset_root
        ...
        for node in ordered_nodes:
            ...
            if destination.exists():
                ...
                if node.source in {SOURCE_REPO, SOURCE_GIT_REPO}:
                    ...
                    continue
                backup_root = Path(tempfile.mkdtemp(prefix=f"ai-kit-backup-{node.id}-"))
