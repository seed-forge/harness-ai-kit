...
def project_add_success_lines(*, asset_kind: str, asset_id: str, manifest_path: Path, no_install: bool) -> list[str]:
    ...
    if no_install:
        lines.append("Next: run `ai-kit sync` to apply the updated project declaration.")
    return lines
...
def project_remove_success_lines(*, asset_kind: str, asset_id: str, manifest_path: Path, no_install: bool) -> list[str]:
    ...
    if no_install:
        lines.append("Next: run `ai-kit sync` to reconcile the updated project declaration.")
    return lines
...
def project_remove_followup_lines(*, asset_kind: str, asset_id: str, removed_skills: Sequence[str], removed_assets: Sequence[str]) -> list[str]:
    ...
    if asset_kind == "cli":
        lines.append(f"Note: project remove does not uninstall the environment-level CLI package `{asset_id}`. Use `ai-kit uninstall cli {asset_id}` if needed.")
    if asset_kind == "mcp":
...
def cli_install_applied_lines(
    *,
    ...
    if include_operator_hint:
        lines.append("Hint: install `ai-kit-ops` separately if you want the operator workflow skill.")
        lines.append("Hint: run `ai-kit install skill ai-kit-ops`.")
    lines.append("Next: run the installed CLI directly or `ai-kit doctor`.")
    return lines
...
def publish_skill_success_lines(
    *,
    ...
    return [
        ...
        f"Next: verify with `ai-kit install skill {skill_id} --dry-run`.",
    ]
...
def publish_asset_success_lines(
    *,
    ...
    return [
        ...
        f"Next: verify with `ai-kit show {asset_kind} {asset_id}`.",
    ]
...
def publish_cli_success_lines(
    *,
    ...
    return [
        ...
        f"Next: verify with `ai-kit install cli {cli_id} --dry-run`.",
    ]
...
def release_skill_build_success_lines(*, skill_id: str, archive_path: Path) -> list[str]:
    return [
        ...
        f"Next: run `ai-kit release skill-publish {skill_id} --dry-run`.",
    ]
...
def release_check_success_lines() -> list[str]:
    return [
        ...
        "Next: run `ai-kit release publish --dry-run` or publish for real.",
    ]
...
def create_scaffold_success_lines(*, subject: str, path: Path) -> list[str]:
    if subject == "skill":
        return [
            ...
            "Next: fill in the skill content, then run `ai-kit submit skill ...`.",
        ]
    if subject == "cli":
        return [
            f"Success: created CLI scaffold -> {path}",
            "Next: implement the CLI tasks, then run `ai-kit submit cli ...`.",
        ]
    return [
        f"Success: created {subject} scaffold -> {path}",
        f"Next: fill in the content, then run `ai-kit submit {subject} ...`.",
    ]
...
def config_set_success_lines(*, config_path: Path) -> list[str]:
    return [
        ...
        "Next: run `ai-kit doctor` or `ai-kit bootstrap`.",
    ]
...
def init_success_lines(*, checkout_message: str, config_path: Path) -> list[str]:
    return [
        ...
        "Next: run `ai-kit doctor` or `ai-kit list assets`.",
    ]
...
def bootstrap_success_lines(*, checkout_message: str) -> list[str]:
    return [
        f"Success: {checkout_message}",
        "Next: run `ai-kit doctor` or `ai-kit list`.",
    ]
...
def init_project_success_lines(*, manifest_path: Path) -> list[str]:
    return [
        ...
        "Next: run `ai-kit sync` or `ai-kit doctor sources` in this project.",
    ]
