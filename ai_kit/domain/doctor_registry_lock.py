...
from ai_kit.domain.policies import SOURCE_GIT_REPO, SOURCE_REGISTRY, SOURCE_REPO

MAINTAINER_DOC = "docs/MAINTAINER-SKILL-LOCK-REGISTRY.md"


def is_ai_kit_repo_root(path: Path) -> bool:
    return (path / "cli" / "ai_kit").is_dir() and (path / "skills").is_dir()


def embedded_ai_kit_skill_checkouts(project_root: Path, root_ids: list[str]) -> dict[str, Path]:
    """Find nested ai-kit skill checkouts inside a consumer workspace."""
    ...
    for root_id in root_ids:
        ...
        candidates = [
            project_root / "工程规范" / "ai-kit" / "skills" / base_id,
            project_root / "ai-kit" / "skills" / base_id,
        ]
        ...
        if root_id in found:
            continue
        for skill_md in project_root.glob(f"**/ai-kit/skills/{base_id}/SKILL.md"):
            kit_root = skill_md.parent.parent.parent
            if kit_root.name == "ai-kit" and is_ai_kit_repo_root(kit_root):
    ...
    for root_id in manifest_skill_ids:
        ...
        if checkout_path is not None and not in_repo_maintainer_context:
            hint = (
                ...
                f"before `ai-kit install --runtime codex --scope project --refresh-lock`."
            )
            if lock_node is not None and lock_node.source == SOURCE_REPO:
                errors.append(
                    f"{root_id}: embedded ai-kit checkout conflicts with registry lock refresh "
                    f"(lock source=repo-checkout). {hint} See {MAINTAINER_DOC}."
                )
            else:
                warnings.append(
                    f"{root_id}: embedded ai-kit checkout detected at `{checkout_path.relative_to(project_root)}`. "
