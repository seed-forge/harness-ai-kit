    ...
    def _load_manifest_for_scope(
        ...
        if scope == "global":
            ...
            if global_path.exists():
                from ai_kit.domain.manifest_ops import load_project_manifest
        ...
        try:
            ...
            if request.asset_kind == "skill":
                raise ValueError(
                    ...
                    "Use --source-ref <git-url> or add a known ai-kit skill."
                ) from exc
            raise
        self._ports.save_project_manifest(manifest_path, manifest)
        return ProjectAddResult(
    ...
    def _add_asset_to_manifest(
        ...
        if request.asset_kind in MANAGED_ASSET_KINDS:
            ...
            if repo_root is None:
                raise FileNotFoundError("A local ai-kit repository is required to add managed assets.")
