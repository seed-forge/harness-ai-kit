...
from ai_kit.domain.models.constants import (
...
from ai_kit.domain.dependency_expansion import current_cli_versions, ordered_unique
from ai_kit.domain.inventory import (
    load_cli_metadata_for_record,
    load_skill_metadata_for_record,
)
from ai_kit.infrastructure.config_io import resolve_repo_root_if_available
from ai_kit.infrastructure.cli_installer import install_cli_packages
from ai_kit.infrastructure.git_ops_extra import maybe_sync_repo
...
from ai_kit.domain.inventory import load_combined_cli_inventory, load_managed_asset_inventory
...
from ai_kit.domain import project_sync as _domain_project_sync
from ai_kit.domain import project_sync_results
from ai_kit.product import active_product_profile as _active_product_profile
    ...
    if selected_source == pm.SOURCE_PUBLIC_REGISTRY:
        ...
        if not registry_index_url:
            raise ValueError(
                ...
                "Set `--public-skill-registry-index-url` via `ai-kit config set` or omit `--from public-registry`."
            )
...
def fanout_canonical_to_runtime(
    ...
    Args:
        ...
        managed_skill_ids: lockfile 历史追踪的 skill 全集
            ...
            是 ai-kit 放置的。为 None 时回退到旧行为（按 SKILL.md 判断）。
    """
    installed = []
    ...
    # 保留非 ai-kit 管理的自定义 skill（不删未知目录）。
