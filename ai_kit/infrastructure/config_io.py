...
from ai_kit.domain.models import CliConfig
from ai_kit.domain.models.config import PublishConfig, IdentityConfig, DefaultsConfig, VALID_ROLES
...
def _migrate_json_to_yaml(config_dir: Path) -> None:
    ...
    try:
        ...
        print(
            f"[ai-kit] Migrated {json_path.name} → {yaml_path.name} "
            f"(backup: {backup.name})",
            file=sys.stderr,
        )
    except Exception as exc:
        print(
            f"[ai-kit] WARNING: Failed to migrate {json_path} → {yaml_path}: {exc}",
            file=sys.stderr,
        )
...
def resolve_repo_root(repo_root_arg: str | None, config: CliConfig, cwd: Path | None = None) -> Path:
    ...
    raise FileNotFoundError(
        "Unable to resolve ai-kit repository root. Run bootstrap first or pass --repo-root."
    )
