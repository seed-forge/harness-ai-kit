...
Loads and merges configuration for a given asset from three priority layers:
  ...
  L2: ~/.ai-kit/.env.tak           (optional sensitive values, KEY=VALUE)
      System environment variables      (matched via env_var declarations)
      ~/.ai-kit/config.yaml        (user global config, HIGHEST user-level priority)
...
def state_dir(asset_id: str, *, create: bool = True) -> Path:
    """Return ``~/.ai-kit/state/<asset_id>/``, creating it if *create* is True.
    ...
    """
    d = Path.home() / ".ai-kit" / "state" / asset_id
    if create:
...
def load_asset_config(
    ...
    *,
    ...
    user_config_path:
        Path to ``~/.ai-kit/config.yaml``.  ``None`` skips L2 YAML.
    env_tak_path:
        Path to ``~/.ai-kit/.env.tak``.  ``None`` skips .env layer.
    cli_overrides:
